...
<Step title="Log In and View Dashboard">
...
<Step title="Verify an Agent Action">
...
The API checks six things in a single call:
...
4. **Asset restrictions** — Is the asset on the agent's allowlist / not on the denylist?
5. **Chain restrictions** — Is the chain on the agent's allowlist / not on the denylist?
6. **Transaction limits** — Is the amount within the agent's configured limits?

</Step>

<Step title="Review Verification History">
...
Procta sends real-time webhook notifications for every verification decision. Your webhook secret (`whsec_...`) was generated at registration.

1. Set your **Webhook URL** during registration or update it later
2. Store the **webhook secret** securely — it's used to verify webhook signatures
...
app.post('/webhooks/procta', (req, res) => {
  ...
  // Verify HMAC-SHA256 signature using your webhook secret
  // See the Webhooks reference for full verification code

  const { event, data } = req.body
  console.log(`Verification ${data.decision}: ${data.kyaId} -> ${data.action}`)

  res.status(200).json({ received: true })
})
```

Webhooks include the full verification payload (decision, scopes, amounts, chain) and are retried up to 3 times with exponential backoff. See the [Webhooks reference](/webhooks#relying-party-webhooks) for details.

</Step>

<Step title="Look Up an Agent">
...
<Step title="Verify Credentials Directly">
...
| Webhook events | N/A (automatic) | Delivered to your `webhookUrl` |
